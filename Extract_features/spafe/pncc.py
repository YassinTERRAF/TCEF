...
def compute_pncc(
    ...
    power=2,
    window: Optional[SlidingWindow] = SlidingWindow(0.025, 0.01, "hamming"),
    nfilts: int = 40,
    nfft: int = 1024,
    ...
    # DCT(.)
    pnccs_all = dct(x=V, type=dct_type, axis=1, norm="ortho")
    pnccs = pnccs_all[:, 1:1+num_ceps]  # Skip the 0th coefficient and take the next `num_ceps` coefficients.
